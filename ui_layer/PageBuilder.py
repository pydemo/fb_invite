...
from ui.include.utils import  fstring, fstring2, exception
import wx.lib.agw.aui as aui
#from include.log_tools import ex

from ui.include.utils import evt_stacktrace
...
class PageBuilder(object):
    ...
    def __init__(self, layout_fn):
        global code
        code=[]
        assert isfile(layout_fn), layout_fn
    ...
    def buildPageLayout(self):
        ...
        for sect, scfg in uilyt.items():
            ...
            for cref  in [x for x in scfg if not ':_' in x]:
                ...
                cvis  	= ccfg.get("captionVisible", None)
                ...
                if 1:
                    ...
                    rmod=splitext(mpath.replace('\\','.'))[0] #.rstrip('.py')
                    code.append(f"""
        #
        #{winname} | {winkey}	
        #{mpath}
        ...
        BestSize(wx.Size(%s,%s)).MinSize(wx.Size(%s,%s)).""" % tuple(bsize+bsize) if bsize else ''
                    ...
                    cmd=f"""
        self.mgr.AddPane(win,aui.AuiPaneInfo().{sect}().Layer({layer}).{bestsize}CaptionVisible({captionvisible}).Caption("{winname}").
        CloseButton({closebutton}).Name("{winname}"))"""
                    #pp(cmd)
                    #e()
                    code.append(cmd)
                ...
                if children:
                    ...
                    for c_cref in [x for x in children if not ':_' in x]:
                        ...
                        if 1:
                            ...
                            c_rmod=splitext(c_mpath.replace('\\','.'))[0]
                            code.append(f"""
