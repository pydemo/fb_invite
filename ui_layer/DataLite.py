...
import ui.include.images as images

from ui.include.EditMenu import EditMenu

from ui.include.StateMan import StateMan
from ui.include.common import home
from ui.include.Base import Base, reciever
from ui.include.utils import  dict2, exception
from ui.include.common import open_editor
...
from ui.include.utils import open_settings
...
class DataLite(wx.Frame, StateMan,  EditMenu, Controller): 

    def __init__(self, parent, title):
        global prefs, mgr, tb, lctrl
        ...
        if 0:
            from ui.include.ImportButton import ImportButton
    ...
    def changeLayout(self, message, arg2=None, **kwargs):
        ...
        from ui.include.MainPanel import  MainPanel
        ...
        if self.first_pane:
            ...
            if 0:
                ...
                pnl2 = MainPanel(self)
                mgr.AddPane(pnl2, aui.AuiPaneInfo().\
                Name("dummy").CaptionVisible(False).Center().MinimizeButton(False).CloseButton(False), target=mgr.GetPane(root_pane))
                if 1:
                    pane= mgr.GetPane("dummy")
                    pane.Hide()
                    pane.Show(False)
                    mgr.ShowPane(pane, False)
                    mgr.DetachPane(pane)
                    mgr.ClosePane(pane)
                    
            self.first_pane = False
            mgr.ShowPane(pnl, True)
            pnl.setFocus()
        else:
            ...
            self.addPane(pnl, pname)
        self.perspective = mgr.SavePerspective()
        mgr.Update()
        self.Thaw()
    def addPane(self,pane, pname, caption=None):
        global mgr, root_pane
        if not caption:
            caption= pname
        mgr.AddPane(pane, aui.AuiPaneInfo().\
                Name(pname).Caption(caption).CaptionVisible(False).Center().MinimizeButton(False).CloseButton(True), target=mgr.GetPane(root_pane))
