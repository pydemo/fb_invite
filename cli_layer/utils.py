...
def init_logging(d=0):
    ...
    if 1:
        ...
        logging.getLogger('zeep.cache').setLevel(logging.WARNING)
...
def timer2(*arguments):
    def decorator(function):
        def wrapper(*args, **kwargs):
            ...
            if 1: #not apc.quiet:
                file, log =arguments
                log.info("ELAPSED |%s | %s |---> [ %0.3f ] sec." % (file, function.__name__, (end - start)))
            return result
        return wrapper
    return decorator
class dict2(dict):                                                              
...
def get_module_loc(**kwargs):
    ...
    pipeline	= convert_path(kwargs['pipeline'])
